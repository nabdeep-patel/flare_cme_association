    ...
    for sampling ,sampling_name in zip([SMOTE, RandomOverSampler, RandomUnderSampler, 'class_weight'], ['SMOTE', 'RandomOverSampler', 'RandomUnderSampler', 'class_weight']):
        ...
        for time_step in time_steps:
            ...
            for solver in ['svd', 'lsqr', 'eigen']:
                if solver == 'svd':
                    shrinkage_values = [None]
                else:
                    shrinkage_values = np.arange(0, 1, 0.01)
                for shrinkage in shrinkage_values:
                    ...
                    for train_index, test_index in skf.split(xdata, ydata):
                        ...
                        if sampling_name == 'class_weight':
                            ...
                            prior = [neg_w/(neg_w+pos_w), pos_w/(neg_w+pos_w)]

                            if solver == 'svd':
                                clf = LinearDiscriminantAnalysis(solver=solver, priors=prior)
                            else:
                                clf = LinearDiscriminantAnalysis(solver=solver, priors=prior,shrinkage=shrinkage)
                        ...
                        print(f"Completed for 'datatype': {datatype}, 'sampling': {sampling_name}, 'time_step': {time_step}, 'solver': {solver}, 'shrinkage': {shrinkage}")
                        
                    # Saving the results
                    final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step,'solver': solver,'shrinkage': shrinkage,
...
results_incorrect.to_csv("lda_incorrect.csv", index=False)
                    ...
                    for train_index, test_index in skf.split(xdata, ydata):
                        ...
                        if sampling_name == 'class_weight':
                            ...
                            if solver == 'svd':
                                ...
                                clf = LinearDiscriminantAnalysis(solver=solver, shrinkage=shrinkage, priors=prior)
                            clf.fit(xtrain, ytrain)
...
results_correct.to_csv("lda_correct.csv", index=False)