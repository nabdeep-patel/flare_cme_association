                    ...
                    for max_features in [2, 3,5,10]:
                        ...
                        for train_index, test_index in skf.split(xdata, ydata):
                            ...
                            if sampling_name == 'class_weight':
                                clf = DecisionTreeClassifier(criterion=criterion, max_depth=max_depth, max_features=max_features, class_weight='balanced', splitter='best')
                                ...
                                clf = DecisionTreeClassifier(criterion=criterion, max_depth=max_depth, max_features=max_features, splitter='best')
                                clf.fit(xtrain, ytrain)
                            
                            # Scores
                            TN, FP, FN, TP = confusion_matrix(ytest, clf.predict(xtest)).ravel()
                            if (((TP+FN) == 0.0) or (FP+TN) == 0.0 or (TP+FP) == 0.0 or (TP+FN) == 0.0):
                            ...
                            print(f"Completed for 'datatype': {datatype}, 'sampling': {sampling_name}, 'time_step': {time_step}, 'criterion': {criterion}, 'max_depth': {max_depth}, 'max_features': {max_features}")

                        # Saving the results
                        final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step, 'criterion': criterion, 'max_depth': max_depth, 'max_features': max_features,
...
# Dataframe
dt_incorrect = pd.DataFrame(final_results)
dt_incorrect.to_csv("dt_incorrect.csv", index=False)
                ...
                for max_depth in [2,6,10,12]:
                    for max_features in [2, 3,5,10]:
                        ...
                        final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step, 'criterion': criterion, 'max_depth': max_depth, 'max_features': max_features,
                            ...
                            'pod_mean': np.nanmean(pod_scores),'pod_std': np.nanstd(pod_scores)})
# Dataframe
dt_correct = pd.DataFrame(final_results)
dt_correct.to_csv("dt_correct.csv", index=False)