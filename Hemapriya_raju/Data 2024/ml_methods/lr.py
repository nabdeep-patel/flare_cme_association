    ...
    for sampling ,sampling_name in zip([SMOTE, RandomOverSampler, RandomUnderSampler, 'class_weight'], ['SMOTE', 'RandomOverSampler', 'RandomUnderSampler', 'class_weight']):
        ...
        for time_step in time_steps:
            ...
            for penalty in ['l1', 'l2']:
                for C in [0.001,0.01,0.05,0.1,1.5,10]:
                    ...
                    for train_index, test_index in skf.split(xdata, ydata):
                        ...
                        if sampling_name == 'class_weight':
                            clf = LogisticRegression(penalty=penalty, C=C, solver=solver, class_weight='balanced', random_state=42)
                        ...
                        print(f"Completed for 'datatype': {datatype}, 'sampling': {sampling_name}, 'time_step': {time_step}, 'penalty': {penalty}, 'C': {C}")

                    # Saving the results
                    final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step, 'penalty': penalty, 'C': C,
...
# Dataframe
lr_incorrect = pd.DataFrame(final_results)
lr_incorrect.to_csv("lr_incorrect.csv", index=False)
            ...
            for penalty in ['l1', 'l2']:
                for C in [0.001,0.01,0.05,0.1,1.5,10]:
                    ...
                    final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step, 'penalty': penalty, 'C': C,
                        ...
                        'pod_mean': np.nanmean(pod_scores),'pod_std': np.nanstd(pod_scores)})
# Dataframe
lr_correct = pd.DataFrame(final_results)
lr_correct.to_csv("lr_correct.csv", index=False)
