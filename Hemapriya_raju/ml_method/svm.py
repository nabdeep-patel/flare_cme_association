                ...
                for gamma in [1, 0.1, 0.01, 0.07, 0.001, 0.005, 0.0001]:
                    ...
                    for train_index, test_index in skf.split(xdata, ydata):
                        ...
                        if sampling_name == 'class_weight':
                            ...
                            clf = SVC(class_weight='balanced', C=C, gamma=gamma, kernel='rbf', cache_size=500, max_iter=-1, shrinking=True, tol=1e-8)
                        ...
                        print(f"Completed for 'datatype': {datatype}, 'sampling': {sampling_name}, 'time_step': {time_step}, 'C': {C}, 'gamma': {gamma}")
                        
                    # Saving the results
                    final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step,'C': C,'gamma': gamma,
...
# Save the results
results_incorrect.to_csv("svm_incorrect.csv", index=False)
results_correct.to_csv("svm_correct.csv", index=False)