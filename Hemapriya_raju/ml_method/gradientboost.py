    ...
    for sampling ,sampling_name in zip([SMOTE, RandomOverSampler, RandomUnderSampler, 'class_weight'], ['SMOTE', 'RandomOverSampler', 'RandomUnderSampler', 'class_weight']):
        ...
        for time_step in time_steps:
            ...
            for n_estimators in [5,10,20, 50]:
                for min_samples_split in np.linspace(0.1, 0.5, 4):
                    for min_samples_leaf in np.linspace(0.1, 0.5, 4):
                            ...
                            for max_features in ["log2","sqrt"]:
                                ...
                                for train_index, test_index in skf.split(xdata, ydata):
                                    ...
                                    if sampling_name == 'class_weight':
                                        ...
                                        class_counts = np.bincount(ytrain.astype(int))
                                        neg_w = len(ytrain)/(2*class_counts[0])
                                        pos_w = len(ytrain)/(2*class_counts[1])
                                        weight = np.where(ytrain==0, neg_w, pos_w)
                                        clf = GradientBoostingClassifier(n_estimators=n_estimators,min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, max_depth=max_depth, max_features=max_features, random_state=42)
                                        clf.fit(xtrain, ytrain, sample_weight=weight)
                                    ...
                                    print(f"Completed for 'datatype': {datatype}, 'sampling': {sampling_name}, 'time_step': {time_step}, 'n_estimators': {n_estimators}, 'min_samples_split': {min_samples_split}, 'min_samples_leaf': {min_samples_leaf}, 'max_depth': {max_depth}, 'max_features': {max_features}")
                                    
                                # Saving the results
                                final_results.append({'datatype': datatype,'sampling': sampling_name,'time_step': time_step,'n_estimators': n_estimators,'min_samples_split': min_samples_split, 'min_samples_leaf': min_samples_leaf, 'max_depth': max_depth, 'max_features': max_features,
...
# Save the results
results_incorrect.to_csv("gb_incorrect.csv", index=False)
results_correct.to_csv("gb_correct.csv", index=False)