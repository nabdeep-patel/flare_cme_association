...
for i in [12, 24, 36, 48, 60, 72, 84, 96, 108, 120]:
    ...
    # Loading data
    flare_linked = pd.read_csv(f"G:/My Drive/Latest Research/01-12-2025/data/flare_linked_{i}.csv").iloc[:, :-4]
    only_cme = pd.read_csv(f"G:/My Drive/Latest Research/01-12-2025/data/only_cme_{i}.csv").iloc[:, :-4]
    only_flares = pd.read_csv(f"G:/My Drive/Latest Research/01-12-2025/data/only_flares_{i}.csv").iloc[:, :-4]
    ...
    for name in ["only_flares", "flare_linked", "only_cme"]:
        ...
        for C in [1e-3, 1e-2, 1e-1, 1, 1e1, 1e2, 1e3, 1e4, 1e5]:
            for gamma in [1, 0.1, 0.01, 0.07, 0.001, 0.005]:
